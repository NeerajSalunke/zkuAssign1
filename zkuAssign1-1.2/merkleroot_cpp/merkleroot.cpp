...
void merkleroot_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void merkleroot_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 12406;}
...
uint csoffset = mySignalStart+3555;
for (uint i = 0; i < 1; i++) {
...
uint csoffset = mySignalStart+7095;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "mimc5";
mySubcomponents[aux_create+i] = aux_cmp_num;
MiMCSponge_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1770 ;
aux_cmp_num += 3;
...
uint csoffset = mySignalStart+8865;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "mimc6";
mySubcomponents[aux_create+i] = aux_cmp_num;
MiMCSponge_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1770 ;
aux_cmp_num += 3;
...
uint csoffset = mySignalStart+10635;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "mimc7";
mySubcomponents[aux_create+i] = aux_cmp_num;
MiMCSponge_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1770 ;
aux_cmp_num += 3;
...
merkleroot_2_create(1,0,ctx,"main",0);
merkleroot_2_run(0,ctx);
}
